...
class GroundingDinoSAM3Inference:
    """Cuando se construye, carga Grounding DINO una sola vez. Cuando se llama
    como función, mide la latencia y la VRAM de una inferencia: localiza el
    objeto descrito con texto mediante Grounding DINO y luego segmenta la
    región detectada con SAM3."""
    ...
    def __call__(self, _unused_first_arg, img_path, text_prompt):
        """Acepta un primer argumento que ignora para mantener la firma común
        de las demás funciones de inferencia (model, img_path, prompt)."""
        return self.run(img_path, text_prompt)
 
    def run(self, img_path, text_prompt):
        """Ejecuta Grounding DINO para obtener la mejor caja a partir del texto
        y la usa como prompt para SAM3."""
